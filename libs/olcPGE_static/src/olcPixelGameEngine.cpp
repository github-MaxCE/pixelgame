...
namespace olc
{
    ...
    Pixel::Pixel(float r, float g, float b, float a) :
        n(uint8_t(r * 255) | (uint8_t(g * 255) << 8) | (uint8_t(b * 255) << 16) | (uint8_t(a * 255) << 24))
    {}

    Pixel Pixel::Lerp(const Pixel& p1, const Pixel& p2, float t)
    {
    ...
    olc::rcode PixelGameEngine::Construct(uint32_t screen_w, uint32_t screen_h, uint32_t pixel_w, uint32_t pixel_h, uint8_t flags)
    {
    ...
    void PixelGameEngine::SetScreenSize(uint32_t w, uint32_t h)
    {
        vScreenSize = { w, h };
    ...
    HWButton PixelGameEngine::GetMouse(Mouse b) const
    {
        return pMouseState[b];
    }

    HWButton PixelGameEngine::GetMouse(int b) const
    {
        return pMouseState[b];
    }
    ...
    void PixelGameEngine::olc_UpdateViewport()
    {
        ...
        else
        {
            ...
            if (vViewSize.y > vWindowSize.y)
            {
                ...
                vViewSize.x = (uint32_t)((float)vViewSize.y * wasp);
            }
        }

        vViewPos = (vWindowSize - vViewSize) / 2u;
    }

    void PixelGameEngine::olc_UpdateWindowSize(uint32_t x, uint32_t y)
    {
        vWindowSize = { x, y };
        olc_UpdateViewport();
    }
...
namespace olc
{
    ...
    public:
        ...
        virtual olc::rcode CreateWindowPane(const olc::vu2d& vWindowPos, olc::vu2d& vWindowSize, bool bFullScreen) override
        {
            WNDCLASS wc;
            ...
            if (bFullScreen)
            {
                ...
                vWindowSize = { (unsigned)mi.rcMonitor.right, (unsigned)mi.rcMonitor.bottom };
                vTopLeft.x = 0;
                vTopLeft.y = 0;
            }

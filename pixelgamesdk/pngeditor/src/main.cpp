...
public:
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
        ...
        if (GetWindowSize() / GetPixelSize() != vWindowSize)
        {
            vWindowSize = GetWindowSize() / GetPixelSize();
            vWindowSize.clamp(350, INT32_MAX, 100, INT32_MAX);
            SetScreenSize(vWindowSize.x, vWindowSize.y);
        }
        if(GetKey(olc::CTRL).bHeld)
        {
            int val = std::clamp(GetMouseWheel(), -1, 1);
            if (val < 0)
                zoom >>= -val;
            else
                zoom <<= val;
        ...
        if(GetMouseWheel() != 0)
            std::cout << std::clamp(GetMouseWheel(), -1, 1) << ": " << zoom << '\n';

        if (!GetKey((olc::Key)0).bHeld && GetKey(olc::F1).bPressed && sprite != nullptr)
        {
        ...
        if (!GetKey((olc::Key)0).bHeld && GetKey(olc::F2).bPressed && sprite == nullptr)
        {
            std::cout.setstate(std::ios_base::failbit);
        ...
        if (!GetKey((olc::Key)0).bHeld && GetKey(olc::F3).bPressed)
        {
            std::cout.setstate(std::ios_base::failbit);
        ...
        if (!GetKey((olc::Key)0).bHeld && GetKey(olc::F4).bPressed && sprite == nullptr)
        {
            std::cout.setstate(std::ios_base::failbit);
